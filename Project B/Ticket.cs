        ...
        using (DataBaseConnection db = new DataBaseConnection())
        {
            ...
            var ticketsPerSchedule = userTickets
                ...
                .Select(scheduleGroup => new
                {
                    ...
                    TicketIDs = scheduleGroup.SelectMany(innerGroup =>
                        innerGroup.Select(t => t.ID)
                    ), // Collecting all ticket IDs within the group
            ...
            if (ticketsPerSchedule.Any())
            {
                ...
                foreach (var scheduleInfo in ticketsPerSchedule)
                {
                    foreach (var ticket in userTickets)
                    {
                        Console.ForegroundColor = ConsoleColor.Green;
                        Console.WriteLine(
                            $"Film: {scheduleInfo.MovieName}, Reserveringnummer: {string.Join(", ", ticket.ReservationNumber)} , Tickets gekocht: {scheduleInfo.TicketCount}, Totale Prijs: {scheduleInfo.TotalPrice}, Datum gekocht: {scheduleInfo.DateBought}"
                        );
                        Console.ResetColor();
                    }
                }
