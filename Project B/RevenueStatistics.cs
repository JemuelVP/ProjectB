...
public class RevenueStatistics
{
    ...
    public static List<dynamic> GetTotalTicketsPerSeatType(int movieID)
    {
        using DataBaseConnection db = new();
        var seatTypeCounts = db
            .Ticket.Where(t => t.Movie_ID == movieID)
            .Join(
                db.Chair,
    ...
    public static void GenerateCSVFile(string inputEmail)
    {
        ...
        {
            writer.WriteLine(
                "Titel, Aantal ClassicSeats, Aantal ExtraBeenRuimte, Aantal LoveSeats, Totale prijs"
            );
            ...
            foreach (var movie in adminOverview)
            {
                ...
                writer.WriteLine(
                    $"{movie.Title}, {countClassicSeats}, {countExtraLegRoom}, {countLoveSeats}, {totalPricePerMovie} euro"
                );
            }
            writer.Flush();
        }
